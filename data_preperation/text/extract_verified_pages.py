...
df = pd.read_csv('data/final_pages.csv')

# filter for pages with sdgs and in english
df = df[df['sdgs'].notna() & (df['information'] == 'yes') & (df['language'] == 'english')]

df = df[['file_name', 'page', 'sdgs']]
...
df.to_csv('data/final_testing_data.csv')
